...
class ImageAugmentorPixelShift(BaseEstimator,TransformerMixin):
...
class TrainedTransformer(BaseEstimator,TransformerMixin):
    def __init__(self,transformer,**kwargs):
        '''
        Wrapper for an already trained transformer to prevent the transformer
        being fitted unintended.
        '''
        self.transformer = transformer
        self.kwargs = kwargs
        if kwargs is not None: self.transformer.set_params(**self.kwargs)
        return None
    def fit(self,X,y=None):
        return self
    def transform(self,X,y=None):
...
class TransformerWrapper(BaseEstimator,TransformerMixin):
    def __init__(self,transformer,turn_on=True,**kwargs):
        '''
        Wrapper for any transformer to allow turning on or off in a pipeline.
        '''
        self.transformer = transformer
        self.kwargs = kwargs
